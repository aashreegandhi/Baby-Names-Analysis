...
Log-transformed Frequency: Because naming data is highly skewed (long-tail distribution), counts are log-transformed for statistical normality in model diagnostics.  

How to Run the Analysis: Using R, run babynames_analysis.R.  
Data Loading: The script uses the tidytuesdayR package with an automated GitHub fallback if the API is unavailable.  
Outputs:  Console:  Statistical summaries for all 4 regression models.  
Folder: An analysis_plots/ directory will be created containing 6 high-resolution PNG visualizations of the results.  
Dependencies: This project utilizes the pacman manager to handle the following libraries:  
tidyverse: Data wrangling and visualization.  
tidytuesdayR: Data acquisition.  
broom: Statistical model tidying.  
modelr: Diagnostic mapping.  
scales: Axis formatting.  
Diagnostics: To ensure robustness, the script includes a Normal Q-Q Plot of residuals. The diagnostics confirm that the error terms follow a normal distribution, validating the use of OLS regression for these cultural trends.
